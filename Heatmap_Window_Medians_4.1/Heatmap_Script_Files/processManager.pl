...
my $num_valid_chroms=keys(%file_sizes); ##This will remove all chromosomes not found in peak file
$num_processors=min($num_cores,4,$num_valid_chroms);
...
for ( my $count = 1; $count <= $num_processors; $count++) {
       ...
       my $pid = fork();
        ...
        push(@children, $pid);
              ...
              # @args=($count, $cur_wig_subfolder, $path_to_wig_files, $output_dir, $num_windows, $dist_each_dir, @{$processors{$count}}); #array of chroms assigned to pid, pid, cur-wig-subfolder, path_to_wig_files, output directory

if(defined($chroms[$count-1])){
@args=($count, $cur_wig_subfolder, $path_to_wig_files, $output_dir, $num_windows, $dist_each_dir, $chroms[$count-1]); #array of chroms assigned to pid, pid, cur-wig-subfolder, path_to_wig_files, output directory
...
sub runWindowMedians {
     my $p_num=$_[0];

     @ARGV=(@_);
    
 eval { require "$SCRIPTS_PATH/getWindowMedians.pl" };
 return $p_num;
