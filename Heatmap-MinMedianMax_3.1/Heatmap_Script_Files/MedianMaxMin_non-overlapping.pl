...
if($flag>0){ splice @peak_starts, 0, $flag;splice @peak_ends, 0, $flag;$flag=0;} #remove peaks that occur before current wig coordinate
...
$peak_end=$peak_ends[$i];
 ...
 if(eof){ #if end of file is reached
   ...
   push @{$wig_signal_vals{$peak}}, $wig_signal_val if $peak_start<=$wig_coord and $peak_end>=$wig_coord;
        ...
        calcMedianMaxMin($wig_signal_vals{$peak},$peak,$pnum,$chr);
        $flag++; #can now remove this peak from peak list
...
}elsif ($wig_line=~m/fixedStep|variableStep/){
