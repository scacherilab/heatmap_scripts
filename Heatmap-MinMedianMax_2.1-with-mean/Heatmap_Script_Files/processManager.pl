...
for ( my $count = 1; $count <= min($#chroms+1,$num_processors); $count++) {
       ...
       my $pid = fork();
        ...
        push(@children, $pid);
               ...
               @args= ($count, $cur_sgr_subfolder, $path_to_sgr_files, $output_dir, $chroms[$count-1]); #array of chroms assigned to pid, pid, cur-sgr-subfolder, path_to_sgr_files, output directory
               runMinMedianMax(@args);
...
my $p_num=$_[0];
      ...
      eval { require "$SCRIPTS_PATH/MedianMaxMin.pl" };
